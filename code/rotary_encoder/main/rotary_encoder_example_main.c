...
static void timer_evt_task(void *arg) {
  while (1) {
    ...
    if (xQueueReceive(timer_queue, &ele, pdMS_TO_TICKS(2000))) {
      float rotations = total_event_count / 12.0;
      printf("wheel turned %f times\n", rotations);
      speed = rotations * (0.052 * 3.14159);
      printf("wheel speed is %f m/s\n", speed);
...
static void display_task(void) {
    ...
    while (1) {
        displaySpeed = speed;
        if (displaySpeed == 0.0) {
            displaybuffer[0] = sevenSegFont[10];
            displaybuffer[1] = sevenSegFont[0];
            displaybuffer[2] = sevenSegFont[0];
            displaybuffer[3] = sevenSegFont[0];
        }
        else {
            factor = 1000;
            displaySpeed = displaySpeed * factor;
            holdSpeed = displaySpeed / factor;
            displaybuffer[0] = sevenSegFont[holdSpeed + 10];
            displaySpeed = displaySpeed - (holdSpeed * factor);
            factor = factor / 10;
            for (i = 1; i < 4; i++) {
                holdSpeed = displaySpeed / factor;
                displaybuffer[i] = sevenSegFont[holdSpeed];
                displaySpeed = displaySpeed - (holdSpeed * factor);
                factor = factor / 10;
            }
        }
...
void app_main(void)
{
    ...
    xTaskCreate(rotary_task, "rotary_task", 2048, NULL, 5, NULL);
